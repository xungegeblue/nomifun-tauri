...
- 安全实现路径（建议）：新增客户端可设、不被剥离的 `extra.autoOrchestration` 构建位；工厂在该位为真（或全局 `nomi.autoOrchestration` 客户端偏好为真）时，**服务端**将 `overrides.desktop_gateway = true` 并注入 lead 能力（复用既有 gateway profile，最好用 lite/orchestration 受限档而非全量总管）。需配套工厂单测（含/不含编排工具）+ 会话能力项 UI（`sessionCapabilityItems.tsx`，与 cron/AutoWork/IDMM 并列）+ 全局设置开关（`configKeys.ts`）。因涉及能力授予的安全边界，留作独立、可测的下一步，未在本轮夜间自主实施中冒进。
