...
const useNomiDraft = getSendBoxDraftHook('nomi', { _type: 'nomi', atPath: [], content: '', uploadFile: [] });
...
const TeamChatEmptyState: React.FC<Props> = ({ conversation_id, icon, isLeader = false }) => {
  ...
  // The draft store is keyed by the string-form conversation id (same boundary the
  // *SendBox components use: `String(conversation_id)`), so coerce here too.
  const draftKey = String(conversation_id);
  const acpDraft = useAcpDraft(draftKey);
  const nomiDraft = useNomiDraft(draftKey);
  const nanobotDraft = useNanobotDraft(draftKey);
  const remoteDraft = useRemoteDraft(draftKey);
  const openClawDraft = useOpenClawDraft(draftKey);
  const setContentByKind = {
    acp: (text: string) => acpDraft.mutate((prev) => ({ ...prev, content: text })),
    nomi: (text: string) => nomiDraft.mutate((prev) => ({ ...prev, content: text })),
