/**
 * AssistantListPanel — Renders assistants as a responsive card grid with a
 ...
 */
import { filterAssistantsByTags, type TagFilterState } from './assistantUtils';
import { useLayoutContext } from '@/renderer/hooks/context/LayoutContext';
import type { AssistantTag } from '@/common/types/agent/assistantTypes';
import type { AssistantListItem } from './types';
import AssistantCard from './AssistantCard';
import AssistantTagFilterBar from './AssistantTagFilterBar';
 ...
 * 面板被一级 rail + 二级 ContentSider 占去宽度,视口宽 ≠ 面板可用宽。助手卡片
...
const AssistantListPanel: React.FC<AssistantListPanelProps> = ({
  ...
  // Depends on `assistants` so it re-runs after async data loads and refs are
  ...
  const filteredAssistants = useMemo(
    () => filterAssistantsByTags(assistants, search_query, tagFilter, localeKey),
    [assistants, search_query, tagFilter, localeKey]
  );
            ...
            <div className='min-w-0'>
              ...
              <p className='mt-8px mb-0 max-w-[680px] text-14px text-t-secondary leading-relaxed'>
                {t('settings.assistantsListDescription', {
                  defaultValue:
                    'Build task-specific assistants by combining an AI agent with custom rules and skills.',
                })}
              </p>
            </div>
          ...
          <div className='grid gap-12px' style={{ gridTemplateColumns: CARD_GRID_COLS }}>
            {filteredAssistants.map((assistant) => (
              <AssistantCard
                ...
                highlighted={highlightedId === assistant.id}
                cardRef={cardRefSetter(assistant.id)}
              />
            ))}
          </div>
        ) : (
