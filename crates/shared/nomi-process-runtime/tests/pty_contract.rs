...
use nomi_process_runtime::{
    ...
    ProcessPolicy, NormalizedProcessRequest, OutputCursor, OutputStream, PollResult,
...
async fn start_pty(
    ...
    args: &[&str],
) -> Result<nomi_process_runtime::ProcessHandle, ProcessError> {
...
async fn consecutive_pty_sessions_do_not_lose_quick_exit_output() {
    ...
    let ProcessOutcome::Exited { code, output, .. } = second_outcome else {
...
async fn resize_after_terminal_close_fails_truthfully() {
    ...
    let outcome = wait_for_terminal(&supervisor, &handle).await;
    assert!(matches!(outcome, ProcessOutcome::Exited { code: Some(0), .. }));
