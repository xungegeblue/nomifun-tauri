...
async fn start_watch_and_detect_change() {
    ...
    svc.start_watch("owner-a", file.to_str().unwrap()).await.unwrap();
...
async fn stop_watch_stops_events() {
    ...
    svc.start_watch("owner-a", path_str).await.unwrap();
    settle().await;

    svc.stop_watch("owner-a", path_str).await.unwrap();
...
async fn stop_all_watches_clears_file_watches() {
    ...
    let (svc, recorder) = make_service();
    svc.start_watch("owner-a", file_a.to_str().unwrap()).await.unwrap();
    svc.start_watch("owner-a", file_b.to_str().unwrap()).await.unwrap();
    settle().await;

    svc.stop_all_watches("owner-a").await.unwrap();
    recorder.take_events();
...
async fn shared_file_watch_keeps_each_owner_isolated_until_they_stop() {
    ...
    svc.start_watch("owner-a", path).await.unwrap();
    svc.start_watch("owner-b", path).await.unwrap();
    settle().await;
    ...
    svc.stop_watch("owner-a", path).await.unwrap();
    settle().await;
    std::fs::write(&file, "v3").unwrap();
    settle().await;
    let owners = recorder.take_owners();
    assert!(!owners.is_empty());
    assert!(owners.iter().all(|owner| owner == "owner-b"));
}

#[tokio::test]
async fn watch_nonexistent_file_returns_error() {
    let (svc, _recorder) = make_service();
    let result = svc.start_watch("owner-a", "/tmp/nonexistent_12345.txt").await;
...
async fn office_watch_detects_docx() {
    let dir = tempfile::tempdir().unwrap();
    let (svc, recorder) = make_service();
    svc.start_office_watch("owner-a", dir.path().to_str().unwrap())
        .await
        .unwrap();
    settle().await;
...
async fn stop_office_watch_stops_events() {
    ...
    svc.start_office_watch("owner-a", ws).await.unwrap();
    settle().await;

    svc.stop_office_watch("owner-a", ws).await.unwrap();
    recorder.take_events();
...
async fn shared_office_watch_remains_active_for_the_other_owner() {
    ...
    svc.start_office_watch("owner-a", workspace).await.unwrap();
    svc.start_office_watch("owner-b", workspace).await.unwrap();
    settle().await;
    ...
    svc.stop_office_watch("owner-a", workspace).await.unwrap();
    settle().await;
